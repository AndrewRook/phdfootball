...
if __name__ == "__main__":
    ...
    ax = plt.figure().add_subplot(111)
    ax.bar(reception_bins[:-1]+0.5,reception_stats[reconstruct_id,:]*reception_sums[reconstruct_id],width=reception_bins[1:]-reception_bins[:-1],facecolor='gray',edgecolor='black',alpha=1,label='Data')
    ...
    ax.text(ax.get_xlim()[0] + (ax.get_xlim()[1]-ax.get_xlim()[0])/40.,ax.get_ylim()[1] - (ax.get_ylim()[1]-ax.get_ylim()[0])/20.,"{0:s}".format(names[reconstruct_id],positions[reconstruct_id]),ha='left',va='center')
    ...
    axup.grid()
    axup.plot(evals_xvals,evals,ls='-',color='black')
    axup.set_ylabel('Normalized Eigenvalues')
    ...
    axdown.grid()
    axdown.plot(evals_xvals,evals_cs*100.,color='black')
    axdown.set_xlabel('Eigenvalue Number')
    axdown.set_ylabel('% Variance Explained')
    axdown.set_ylim(30,100)
    ...
    for i in range(len(axs)):
        ...
        #axall_maxes.append(np.max(pca_components[i]/np.max(pca_components[i])))
        #axall_mins.append(np.min(pca_components[i]))
    ...
    #axall.set_ylim(np.min(axall_mins)-0.1,np.max(axall_maxes)+0.1)
